# ReachMyDevice — Phase 1 (Spike) Report
...
- **Transport loopback** (`cargo test -p rmd-transport`): two peers ICE-connect + establish
  DTLS-SRTP and exchange video + bidirectional data over `127.0.0.1` in **~0.46 s** per run; stable across
  5 consecutive runs.
- **End-to-end media pipeline** (`cargo test -p rmd-session --test pipeline`): real openh264
...
- **Codec roundtrip** (`cargo test -p rmd-codec`): encode→decode of a 128×96 frame passes.
...
2. On machine A (or either): `RMD_SIGNAL_ADDR=0.0.0.0:9000 cargo run --release -p rmd-signal-dev`.
3. On the **host** Mac: `RMD_SIGNAL_ADDR=<relay-ip>:9000 cargo run --release -p rmd-host`
   (grant **Screen Recording** + **Accessibility** when prompted; restart after granting — see
   `docs/macos-permissions.md`).
4. On the **viewer** Mac: `RMD_SIGNAL_ADDR=<relay-ip>:9000 cargo run --release -p rmd-viewer`.
