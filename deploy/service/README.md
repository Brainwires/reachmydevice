...
agent), seeds `~/.config/openreach` (env template + `authorized_keys`), and prints
...
2. On the host, add those ids — one per line — to
   `~/.config/openreach/authorized_keys` (or point `OPENREACH_AUTHORIZED_KEYS`
   elsewhere). Its presence enables enforcement automatically; you can also force
   it with `OPENREACH_REQUIRE_AUTH=1`.
